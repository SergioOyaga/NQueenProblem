...
public class NQueenProblem {
     ...
     public static void runGenetic(int cycles,double P1o, double P2o, double P1c, double P2c,double P1eo, double P2eo,
        ...
        FileWriter fw = new FileWriter("out/rFiles/file.csv");
        fw.append("Type of crossover,Generation,Mean,Best \n");
        FileWriter fw2 = new FileWriter("out/rFiles/file1.csv");
        ...
        for(int i=0;i<cycles;i++){
            ...
            if(i==0){//write the type of cross, the generation number, the mean and the best individual
              write[i]="Order Crossover,"+Integer.toString(i)+","+Double.toString(populationOrder.mean())+","+populationOrder.getBestFitness()+"\n";
            }
            populationOrder=populationOrder.nextGenerationTournament(true,false);//obtains the next generation (order,no elitism)
            write[i+1]="Order Crossover,"+Integer.toString(i+1)+","+Double.toString(populationOrder.mean())+","+populationOrder.getBestFitness()+"\n";
            
            //cycle method
            populationCycle.Pmut1=P1c;//mutation probabilities
            populationCycle.Pmut2=P2c;
            if(i==0){//write the type of cross, the generation number, the mean and the best individual
              write[i]+="Cycle Crossover,"+Integer.toString(i)+","+Double.toString(populationCycle.mean())+","+populationCycle.getBestFitness()+"\n";
            }
            populationCycle=populationCycle.nextGenerationTournament(false,false);//obtains the next generation (cycle,no elitism)
            write[i+1]+="Cycle Crossover,"+Integer.toString(i+1)+","+Double.toString(populationCycle.mean())+","+populationCycle.getBestFitness()+"\n";
            
            //elitism order method
            populationOrderElitism.Pmut1=P1eo;//mutation probabilities
            populationOrderElitism.Pmut2=P2eo;
            if(i==0){//write the type of cross, the generation number, the mean and the best individual
              write[i]+="Elitism Order Crossover,"+Integer.toString(i)+","+Double.toString(populationOrderElitism.mean())+","+populationOrderElitism.getBestFitness()+"\n";
            }
            ...
            write[i+1]+="Elitism Order Crossover,"+Integer.toString(i+1)+","+Double.toString(populationOrderElitism.mean())+","+populationOrderElitism.getBestFitness()+"\n";
        
            //elitism cycle method
            populationCycleElitism.Pmut1=P1ec;//mutation probabilities
            populationCycleElitism.Pmut2=P2ec;
            if(i==0){//write the type of cross, the generation number, the mean and the best individual
              write[i]+="Elitism Cycle Crossover,"+Integer.toString(i)+","+Double.toString(populationCycleElitism.mean())+","+populationCycleElitism.getBestFitness()+"\n";
            }
            ...
            write[i+1]+="Elitism Cycle Crossover,"+Integer.toString(i+1)+","+Double.toString(populationCycleElitism.mean())+","+populationCycleElitism.getBestFitness()+"\n";
        ...
        Board orderBoard= new Board();
        if (populationOrder.Population[0].fitness==0){orderBoard.setTitle("Order");}
        else{orderBoard.setTitle("Order didn't find an optimal solution.");}

        new Controller(orderBoard,populationOrder.Population[0].getGenome());
        for(Population.Candidate c:populationOrder.Population){
            if(c.fitness==populationOrder.Population[0].fitness){
        ...
        Board cycleBoard= new Board();
        if (populationCycle.Population[0].fitness==0){cycleBoard.setTitle("Cycle");}
        else{cycleBoard.setTitle("Cycle didn't find an optimal solution.");}
        new Controller(cycleBoard,populationCycle.Population[0].getGenome());
        for(Population.Candidate c:populationCycle.Population){
            if(c.fitness==populationCycle.Population[0].fitness){
                System.out.print( c.name+ "->");
            }
        }
        System.out.println("\n &&&&&&&&&&&&&\n");
        ...
        Board orderEliteBoard= new Board();
        if (populationOrderElitism.Population[0].fitness==0){orderEliteBoard.setTitle("Elite Order");}
        else{orderEliteBoard.setTitle("Elite order didn't find an optimal solution.");}

        new Controller(orderEliteBoard,populationOrderElitism.Population[0].getGenome());
        for(Population.Candidate c:populationOrderElitism.Population){
            if(c.fitness==populationOrderElitism.Population[0].fitness){
                System.out.print( c.name+ "->");
            }
        }
        System.out.println("\n &&&&&&&&&&&&&\n");
        ...
        Board cycleEliteBoard= new Board();
        if (populationCycleElitism.Population[0].fitness==0){cycleEliteBoard.setTitle("Elite Cycle");}
        else{cycleEliteBoard.setTitle("Elite cycle didn't find an optimal solution.");}
        
        new Controller(cycleEliteBoard,populationCycleElitism.Population[0].getGenome());
        for(Population.Candidate c:populationCycleElitism.Population){
            if(c.fitness==populationCycleElitism.Population[0].fitness){
                System.out.print( c.name+ "->");
            }
        }
        System.out.println("\n &&&&&&&&&&&&&\n");
